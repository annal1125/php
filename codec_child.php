<?php
	...
	$parse_temp = explode( "," , $InputIndex );	
	$codec_type 	= $parse_temp[0];
	$Parent_Index 	= $parse_temp[1];
	$InputIndex 	= $parse_temp[2];
	
	$ArrayCodec	= ( $codec_type == "audio" ) ? $AudioArray : $VideoArray;
	$Ch_Codec	= ( $codec_type == "audio" ) ? "ch_Acodec" : "ch_Vcodec";
	...
	{	
		...
		echo "window.opener.form.$Ch_Codec$Parent_Index.value='$CodecStr';";
		echo "window.close();";
		echo "</Script>";
	}
...
//	$tok = strtok($InputIndex,"/");
	...
	$RightContect = "";
	$Leftcontent = $ArrayCodec;
	
   if( $InputIndex == null )
	{
		$RightContect = $ArrayCodec;
	}       
	else
	{
	    $codectmp = explode( "/" , $InputIndex );
		foreach ($codectmp as $val) 
		{
			if( $ArrayCodec[$val] )
			{
				$RightContect[$val] = $ArrayCodec[$val];
				unset($Leftcontent[$val]);
			}
		}
	}
